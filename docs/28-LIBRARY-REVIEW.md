...
## אימות מוקדם — סוכן Claude עם דפדפן

לעבור לבד על מאות סעיפים זה לא ריאלי. לכן, לפני הסקירה, סוכן Claude עם דפדפן
בודק כל סעיף **פיזית** מול נבו, מאגר החקיקה של הכנסת ו-gov.il — שש בדיקות:
הציטוט קיים כלשונו (V1), האסמכתא נכונה (V2), התקנה בתוקף (V3), השאלה נאמנה לחוק (V4),
"כן" פירושו תקין (V5), וניתן לבדוק בשטח (V6).

```bash
pnpm library:brief                                  # ← build/verification/brief.md
pnpm library:import-verification <תשובת-הסוכן.md>  # ← data/verification/<תאריך>.json
pnpm library:review <batch>                         # הדף מציג את הממצאים
```

**אימות אינו אישור.** הוא מסמן לממונה על מה לעבור — דף הסקירה נפתח בלשונית
"סומנו לבדיקה", וכל כרטיס נושא תג "אומת מול נבו" או את הממצא. `reviewed_by` נכתב רק
מהחלטת הממונה (R16).

**הפלט של הסוכן הוא קלט לא מהימן** (25-AI-SECURITY §3): הוא קרא אתרי אינטרנט. לכן הוא
עובר סכמה קשיחה לפני ייבוא — מזהים קיימים בלבד, ארבעה ערכים מותרים, הסבר חובה לכל
ממצא, מקורות מנבו, מהכנסת ומ-gov.il בלבד — ואף שדה בו לא משנה טיוטה.
...
| 2 · משטחי עבודה וגידור, 2.2 תקנות 8–15 | [Artifact](https://claude.ai/artifact/HEvV9dwhzEqJkHffQDFksD) | 👤 ממתין לסקירה |

עותק מקומי של כל דף נמצא ב-`ממונה בטיחות/סקירת ספרייה/` — עובד בלי חיבור, עם ייצוא החלטות לקובץ.
